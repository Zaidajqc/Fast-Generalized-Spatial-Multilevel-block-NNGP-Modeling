...
fit0 <- multilevelNS_gamma$summary.fitted.values[1:length(y),]
par(mfrow=c(1,1))
plot(y, fit0[,1])
...
get.est0<-function(i){
  precision<-multilevel0.pos[[i]]$Precision_parameter_for_the_Gamma_observations
...
load("/Users/zquiroz/Documents/ZAIDA_MAC/PAPERS/Fast Spatial Multilevel Model (Spatial Statistics)/app_pcPrior/NEWbeijing.RData")
...
n.blocks <- 2^L
dir.save = getwd()
png(paste(dir.save,'/Fulmultilevel_gammaInt_L',n.blocks,'_',M,"_etas.png",sep=""))
...
crit.model<- round(c( dic, waic, LPML, RMSEy, time, RMSP),3)
res=data.frame(name.crit, val=crit.model)
filename2= paste("FULLMULTILEVELrandomint_gamma_summary_",n.blocks, "-",M,".Rdata",sep="")
...
png(paste(dir.save,"/FULLMULTILEVELrandomint_gamma_",n.blocks, "-",M,"gamma.png",sep=""))
...
#DDD<- matrix(NA, dim(train.coord)[1], 2)
#DDD[,1]<- train.coord$Lng
#DDD[,2]<- train.coord$Lat
#distanceMatrix<-spDists(DDD,longlat = T) 
#ndx <- order(distanceMatrix)[1:2370]
#distanceMatrix[ndx[2366:2370]]
...
n.blocks <- 2^L
png(paste(dir.save,"/est_gamma_multilevel1", n.blocks, "-",M,".png",sep=""))
...
n.blocks <- 2^L
dir.save = getwd()
png(paste(dir.save,'/Fulmultilevel_gamma_L',n.blocks,'_',M,"_etas.png",sep=""))
par(mfrow=c(1,3))
image.plot(int.elev1 , #main='spatial random effects',
           xaxs = 'r', yaxs = 'r',
           xlab='Longitude', ylab='Latitude')
...
get.est<-function(i){
  precision<-multilevel1.pos[[i]]$Precision_parameter_for_the_Gamma_observations
...
png(paste(dir.save,"/Fulmultilevel_gamma_L",n.blocks,"_",M,"_PRED.png",sep=""))
ggplot(data = plot3,mapping = aes(x=totalPrice,y=pred))+geom_point()+ geom_abline(colour = "grey50")
dev.off()
...
png(paste(dir.save,"/FULLMULTILEVELrandomcoef_gamma_",n.blocks, "-",M,"gamma.png",sep=""))
par(mfrow=c(1,3))
image.plot(int.elev5 , #main='spatial random effects',
           xaxs = 'r', yaxs = 'r',
           xlab='Longitude', ylab='Latitude')
...
png(paste(dir.save,"/FULLMULTILEVEL_plot10gamma_",n.blocks, "-",M,".png",sep=""))
par(mfrow=c(2,3))
image.plot(int.elev1 , #main='spatial random effects',
           xaxs = 'r', yaxs = 'r',
           xlab='Longitude', ylab='Latitude')
