...
## full multilevel coefficients models - gamma distribution
...
multilevelsim1 <- function(indseed){
  ...
  beta1_true <- -1;beta2_true<- 0.5; beta3_true <- -0.2;
  
  phis_true <- 15; sig2 <- 0.15;  phi<-30; p.cov<-0.5;
  phis_true2 <- 6; sig22 <- 0.1;
  phis_true3 <- 10; sig23 <- 0.05;
  ...
  distM <- as.matrix(dist(coord[,c('horizontal','vertical')])) 
  ...
  cov2<- unlist(sapply(nhouse, function(x) rnorm(x,6,0.5)))
  ...
  nclaims <- sapply(exp(beta1_true + beta2_true*cov2 + beta3_true*cov3 +
                      rep(trueZ,coord$nhouse) +
                      cov2*rep(trueZ2,coord$nhouse) +
                      cov3*rep(trueZ3,coord$nhouse)),
                    function(x) rgamma(1, shape=phi, rate=phi/x ),simplify = T)
  ...
  marg.phi<-inla.tmarginal(function(x){x},m.blocknngp$marginals.hyperpar$`Precision parameter for the Gamma observations`)
  inla.zmarginal(marg.phi)
  ...
  estphi<-inla.emarginal(function(x){x},m.blocknngp$marginals.hyperpar$`Precision parameter for the Gamma observations`)
  ...
  covphi=0; if(phi>= inla.hpdmarginal(0.95,marg.phi)[1] & phi<=inla.hpdmarginal(0.95,marg.phi)[2]) covphi =1
  ...
  names.mean.estparam <- c("est.beta1", "est.beta2", "est.beta3","estsig1", "estphi1", "estsig2", "estphi2", "estsig3", "estphi3", "estphi")
  mean.estparam <- round(c(est.beta, estsig1, estphi1, estsig2, estphi2, estsig3, estphi3, estphi),3)
  names.cov.estparam <- c("cov.beta1", "cov.beta2", "cov.beta3","covsig1", "covphi1", "covsig2", "covphi2", "covsig3", "covphi3", "covphi")
  cov.param <- c(covbeta1, covbeta2, covbeta3, covsig1, covsig2, covsig3, covphi1, covphi2, covphi3, covphi)
  ...
  # save true values
  name.true.param <-  c("beta1_true", "beta2_true", "beta3_true" , "sig1", "phis_true1", "sig2", "phis_true2", "sig3", "phis_true3", "phi")
  true.param <-  c(beta1_true, beta2_true, beta3_true , sig2, phis_true, sig22, phis_true2, sig23, phis_true3, phi)
  ...
  abline(0,1, col="red")
  plot(test$z2, rep(colMeans(pre_Z.2),test.coord$nhouse),xlim=c(-1,1), ylim=c(-1,1))
  abline(0,1, col="red")
  ...
  plot3 <- data.frame( y=y.test,  y_pred=pre.block.nngp2 )
  
  plopt_pred =  ggplot(data = plot3,mapping = aes(x=y,y=y_pred))+
    geom_point(alpha = 1/10)+
    geom_abline(colour = "grey50") #+  coord_flip( xlim = c(0, 500),ylim = c(0,500))

    dir.save=getwd()
  jpeg(paste("Fulmultilevel_gamma_L",nb^2,"_",M,"_PRED.jpeg",sep=""))
  plopt_pred 
  dev.off()
...
save(res=res, file="ALLmultilevel_sim1.Rdata")
...
df8 <- data.frame(cat=res[11:20, 1],
                  Cov_prob= rowSums(res[11:20, -1])/niter)
save(df8=df8, file="coverageprob_sim1.Rdata")
